...
Python 3.13 버전을 사용합니다. `uv`만으로 가상 환경 생성부터 의존성 설치까지 한 번에 구성할 수 있습니다.
...
개발 도구(`ruff`, `pre-commit`)까지 함께 설치하려면 아래 명령을 사용하세요.
...
`uv run <명령어>` 형식으로 실행하면 활성화 없이도 `.venv`를 사용할 수 있습니다.
